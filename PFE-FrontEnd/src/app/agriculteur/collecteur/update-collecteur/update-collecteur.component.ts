...
export class UpdateCollecteurComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.collecteurService
      .getCollecteur(JSON.parse(localStorage.getItem('IdC') || '[]') || [])
      .subscribe((o) => {
        this.collecteur = o;
  ...
  updatecollecteur() {
    ...
    if (this.myForm.get('nomCollecteur')?.value.length >= 4) this.msg1 = 0;
    else this.msg1 = 1;

    if (this.myForm.get('adresse')?.value.length >= 3) this.msg2 = 0;
    else this.msg2 = 1;

    if (this.myForm.get('matricule')?.value.length >= 8) this.msg3 = 0;
    else this.msg3 = 1;

    if (
      this.myForm.get('nomCollecteur')?.value != null &&
      ...
      this.myForm.get('nomCollecteur')?.value.length >= 4 &&
      this.myForm.get('adresse')?.value.length >= 3 &&
        ...
        .subscribe(
          (o) => {
            localStorage.setItem(
              ...
              JSON.stringify([
                'Success',
                'Un collecteur a été modifié avec succès ',
              ])
            );
            console.log(this.collecteur);
            this.onClose();
  ...
  ValidatedForm() {
    this.myForm = new FormGroup({
      nomCollecteur: new FormControl(null, [
        Validators.required,
