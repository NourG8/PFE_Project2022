...
export class UpdateVacheComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.ValidatedForm();
    this.vacheService
      .getVache(JSON.parse(localStorage.getItem('IdVache') || '[]') || [])
      .subscribe((o) => {
        this.vache = o;
  ...
  updateVache() {
    this.vacheService.updateVache(this.vache.idVache, this.vache).subscribe(
      (o) => {
        localStorage.setItem(
          'Toast',
          JSON.stringify(['Success', 'Une vache a été modifiée avec succès'])
        );
        console.log(this.vache);
        this.onClose();
      },
