...
export class CreateBonComponent implements OnInit {
  ...
  save() {
    ...
    if (
      this.myForm.get('quantite')?.value != null &&
      this.myForm.get('prix')?.value != null &&
      this.myForm.get('quantite')?.value != 0 &&
      this.myForm.get('prix')?.value != 0 &&
      this.myForm.get('produit')?.value != null &&
      this.myForm.get('fournisseur')?.value != null &&
      this.myForm.get('quantite')?.value > 0 &&
      this.myForm.get('prix')?.value > 0 &&
      ...
      this.bonService
        ...
        .subscribe((o) => {
          ...
          localStorage.setItem(
            'Toast',
            JSON.stringify(['Success', 'Un bon a été ajouté avec succès'])
          );
          this.onClose();
        });
    }
